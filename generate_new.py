'''
cd /dev/shm/
wget https://s3-eu-west-1.amazonaws.com/bteckenodes/collab_filter/magento_ords.csv
...
cust_reqd.CustomerID = cust_reqd.CustomerID.astype(int)
...
sku_cust = ords_bin.columns.tolist()
for i in sku_encoder1:
    if i not in sku_cust:
        ords_bin[i] = 0

#set the customerID as index, retain only skus from feature matrix and transpose
#cust_prod_bin = ords_bin.set_index('Unnamed: 0')

cust_prod_bin = ords_bin[sku_encoder1]
